document.addEventListener('DOMContentLoaded', () => {
    ...
    // ========== NEWS MANAGEMENT ==========
    const NEWS_API = API_BASE.replace('/matches', '/news');
    const newsForm = document.getElementById('news-form');
    const newsTitle = document.getElementById('news-title');
    const newsDate = document.getElementById('news-date');
    const newsImage = document.getElementById('news-image');
    const newsContent = document.getElementById('news-content');
    const newsEditMode = document.getElementById('news-edit-mode');
    ...
    async function fetchNews() {
        try {
            ...
            if (response.ok) {
                ...
                showNotification('فشل في تحميل الأخبار', 'error');
            }
        } catch (error) {
            showNotification('خطأ في الاتصال بالخادم', 'error');
    ...
    function renderNews() {
        ...
        if (newsData.length === 0) {
            newsList.innerHTML = '<div class="no-matches">لا توجد أخبار حالياً</div>';
            return;
        }

        newsData.forEach((item) => {
            const newsItem = document.createElement('div');
            newsItem.className = 'match-item-admin';
            newsItem.innerHTML = `
                <div class="match-info-admin">
                    <div class="match-teams">
                        <img src="${item.image}" alt="${item.title}" style="width: 60px; height: 40px; object-fit: cover; border-radius: 4px;" onerror="this.src='assets/logo.png'">
                        <div style="display: flex; flex-direction: column;">
                            <span><strong>${item.title}</strong></span>
                            <span style="font-size: 0.8rem; color: #aaa;">${item.date}</span>
                        </div>
                    </div>
                </div>
                <div class="match-actions">
                    <button class="btn-edit" onclick="editNews('${item.id}')">تعديل</button>
                    <button class="btn-delete" onclick="deleteNews('${item.id}')">حذف</button>
                </div>
            `;
    ...
    if (newsForm) {
        newsForm.addEventListener('submit', async (e) => {
            ...
            const isEditMode = newsEditMode.value === 'true';
            ...
            try {
                ...
                if (isEditMode) {
                    ...
                    response = await fetch(`${NEWS_API}/${id}`, {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json' },
                    ...
                    if (response.ok) {
                        ...
                        showNotification('فشل في تحديث الخبر', 'error');
                    }
                } else {
                    // Add new news
                    response = await fetch(NEWS_API, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify(itemData)
                    });
                    ...
                    if (response.ok) {
                        ...
                        newsForm.reset();
                        // Set today's date by default again if needed
                        newsDate.valueAsDate = new Date();
                    } else {
                        showNotification('فشل في إضافة الخبر', 'error');
                    }
                }
    ...
    window.editNews = function (id) {
        ...
        newsEditMode.value = 'true';
        newsEditId.value = id;

        newsSubmitBtn.textContent = 'تحديث الخبر';
        newsCancelBtn.style.display = 'block';
    ...
    window.deleteNews = async function (id) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                showNotification('فشل في حذف الخبر', 'error');
            }
        } catch (error) {
            showNotification('خطأ في الاتصال بالخادم', 'error');
    ...
    function resetNewsForm() {
        newsForm.reset();
        newsEditMode.value = 'false';
        newsEditId.value = '';
        newsSubmitBtn.textContent = 'إضافة الخبر';
